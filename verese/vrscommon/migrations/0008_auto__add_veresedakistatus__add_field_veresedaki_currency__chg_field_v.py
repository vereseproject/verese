...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding field 'Veresedaki.currency'
        db.add_column('vrscommon_veresedaki', 'currency', self.gf('django.db.models.fields.related.ForeignKey')(default=1, to=orm['vrscommon.Currency']), keep_default=False)

        # Renaming column for 'Veresedaki.status' to match new field type.
        db.rename_column('vrscommon_veresedaki', 'status', 'status_id')
        # Changing field 'Veresedaki.status'
        db.alter_column('vrscommon_veresedaki', 'status_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['vrscommon.VeresedakiStatus']))

        # Adding index on 'Veresedaki', fields ['status']
        db.create_index('vrscommon_veresedaki', ['status_id'])


    def backwards(self, orm):
        
        # Removing index on 'Veresedaki', fields ['status']
        db.delete_index('vrscommon_veresedaki', ['status_id'])

        # Deleting model 'VeresedakiStatus'
        db.delete_table('vrscommon_veresedakistatus')

        # Deleting field 'Veresedaki.currency'
        db.delete_column('vrscommon_veresedaki', 'currency_id')

        # Renaming column for 'Veresedaki.status' to match new field type.
        db.rename_column('vrscommon_veresedaki', 'status_id', 'status')
        # Changing field 'Veresedaki.status'
        db.alter_column('vrscommon_veresedaki', 'status', self.gf('django.db.models.fields.IntegerField')())
